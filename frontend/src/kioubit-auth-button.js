...
class KioubitAuthButton extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    const input = this.shadowRootRef.querySelector(`input[name="${name}"]`)
    if (input) input.value = name === 'return' ? this.returnUrl : this.tokenValue
  }

  get returnUrl() {
    const value = this.getAttribute('return')
  ...
  render() {
    this.shadowRootRef.innerHTML = `
      ...
      <form action="https://dn42.g-load.eu/auth/">
        ...
        <input type="hidden" name="token" value="${this.tokenValue}">
        <button type="submit" class="kioubit-btn-dark" part="button">
          <img width="35" height="35" src="data:image/svg+xml;base64,${svgBase64}" alt="Kioubit.dn42 logo" class="kioubit-btn-logo">
